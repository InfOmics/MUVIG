...
snp.pheno.st.ass = cbind(
  snp.pheno.st.ass, data.frame(
    BP = read.table(
      paste(
        paste(
          paste(genotyping.dir, "Individual_View_Datscan/indview_datscan.", sep = ""), 
          "CAUDATE_R", sep = ""
        ), 
        ".assoc.linear", sep = ""
      ), header = T
